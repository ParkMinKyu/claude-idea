...
## 향후 로드맵
- M1: CLI MVP (npm install + 단일 명령)
- M2: GitHub Action 배포
- M3: 비공개 저장소 라이센스 시스템
- M4: 다국어 출력, 마케팅 트윗 자동 생성

## 리스크와 대응
- 큰 릴리스의 커밋 100+ → 토큰 한도 → chunk 처리
- 민감 정보가 커밋 메시지에 있을 수 있음 → 사용자 측 필터 옵션
- OSS는 무료지만 비용 발생 → "OSS license 확인 + rate limit"로 통제
