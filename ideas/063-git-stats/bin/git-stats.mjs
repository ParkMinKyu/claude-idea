...
<script>
// ── Embedded stat & render functions (mirror of server-side) ──
const KST_OFFSET_MS = 9 * 60 * 60 * 1000;
...
function renderStats(commits,contribs){
  ...
  return \`
    <div class="stat"><div class="label">총 커밋</div><div class="value">\${fmt(commits.length)}</div><div class="desc">필터 적용 후</div></div>
...
function apply() {
  ...
  const to = toEl.value;
  const filtered = ALL_COMMITS.filter(c => {
...
document.querySelectorAll('.preset').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (days === 0) {
      fromEl.value = MIN_DATE; toEl.value = MAX_DATE;
    } else {
      const today = new Date();
